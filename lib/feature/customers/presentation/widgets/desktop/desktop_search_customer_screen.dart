...
import '../mobile/customer_orders_list_widget.dart';

class DesktopSearchCustomerScreen extends StatefulWidget {
  const DesktopSearchCustomerScreen({super.key, this.phoneNumber});
  final String? phoneNumber;

  @override
  State<DesktopSearchCustomerScreen> createState() => _DesktopSearchCustomerScreenState();
}

class _DesktopSearchCustomerScreenState extends State<DesktopSearchCustomerScreen>
    with SingleTickerProviderStateMixin {

  late final TabController _tabController;
  late final _searchController = TextEditingController(text: widget.phoneNumber);
  ...
  void initState() {
    ...
    if (widget.phoneNumber != null) context.read<CustomerBloc>().add(SearchForCustomer(widget.phoneNumber!));
  }
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(child: Text(S.current.searchCustomer, style: context.textTheme.titleLarge)),
                    SizedBox(
                      width: 500,
                      ...
                      Expanded(
                        child: BlocBuilder<CustomerBloc, CustomerState>(
                            ...
                            builder: (context, state) {
                              ...
                              return Visibility(
                                visible: context
                                    .select<CustomerBloc, bool>((value) => value.foundCustomer != null),
                                child: Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      RichText(
                                        ...
                                        TextSpan(text:'${S.current.name}: ', style: context.textTheme.bodyMedium),
                                        TextSpan(
                                      ...
                                      RichText(
                                        ...
                                        TextSpan(
                                            text: context.select<CustomerBloc, String>((value) =>
                                                value.foundCustomer?.customerAddress?.fullAddress ?? ''),
                                            style: context.textTheme.titleMedium
                                                ?.copyWith(fontWeight: FontWeight.bold)),
                                      ])),
                                      context.sizedBoxHeightMicro,
                            ...
                            children: [
                              Visibility(
                                visible: context.select<CustomerBloc, bool>(
                                    ...
                                    context.select<CustomerBloc, bool>(
                                        (value) => value.ordersPagination.listItems.isNotEmpty),
                                      ...
                                      builder: (context, state) {
                                        if (state is GetCustomerOrdersLoadingState) {
                                          return ShimmerWidget(width: Get.width, height: Get.height);
                                            ...
                                            (value) => value.ordersPagination.isQuiteEmpty)) {
                                          return const Center(
                                        ...
                                        return CustomerOrdersListWidget(
                                          getOrders: (getMore) => context
                                              .read<CustomerBloc>()
                                              .add(GetCustomerOrders(getMore: getMore)),
                                          orders: context.select<CustomerBloc, List<CustomerOrder>>(
                                            (value) => value.ordersPagination.listItems,
                                          ),
                                          hasMore: context.select<CustomerBloc, bool>(
                                              (value) => value.ordersPagination.hasMore),
                                        );
                                      },
                                    ),
                                    BlocBuilder<CustomerBloc, CustomerState>(
                                      builder: (context, state) {
                                        ...
                                        return MobileTransactionListWidget(
                                          getTransactions: (getMore) => context
                                              .read<CustomerBloc>()
                                              .add(GetCustomerTransactions(getMore: getMore)),
